...
sm_eks_node () {
  ...
  if [ -z "$node_name" ]
  then
    node_name=$(kubectl get nodes | fzf | awk '{print $1}')
  fi
  ...
  local region=$(kubectl config view | yq ".clusters[] | select(.name == \"$cluster\") | .cluster.server" | cut -d '.' -f3)
  local node_id=$(kubectl get node $node_name -o yaml | yq '.spec.providerID' | cut -d '/' -f5)

  local aws_profile=$(kubectl config view  | yq ".users[] | select(.name == \"$cluster\")| .user.exec.env[] | select(.name == \"AWS_PROFILE\") | .value")
